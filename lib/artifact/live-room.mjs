...
export async function decryptLiveRoomEvent(record, { key, roomId, reviewOf, crypto } = {}) {
  const bytes = await decryptArtifactPayload(record, { keyFragment: key, crypto, maxEncryptedBytes: ARTIFACT_COMPRESSED_LIMIT });
  let value;
...
export function createLiveRoomClient({ baseUrl = 'https://share.openplanr.dev', fetchImpl = globalThis.fetch } = {}) {
  ...
  return Object.freeze({
    async create({ envelope, ttl = '7d', crypto } = {}) {
      ...
      const reviewOf = await digestEnvelope(envelope, crypto); const compressed = compressArtifactPayload(envelope); const encrypted = await encryptArtifactPayload(compressed, { crypto });
