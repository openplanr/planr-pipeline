...
function artifactGuardAndBridgeSource({ artifactId, nonce, parentOrigin }) {
  ...
  addEventListener('message',event=>{
    ...
    if(validBase(data,'bridge.challenge',['channel','schemaVersion','type','artifactId','requestId'])){
      send('bridge.challenge-ack',data.requestId);return;
    }
...
export function renderArtifactParentRuntime({
  ...
  const bridgeClient={attach({artifact,frame}){
    ...
    const receive=event=>{
      ...
      const anchor=data.anchor,rect=anchor?.rect,viewport=anchor?.viewport,frozen=artifact.viewport;
    ...
    const rememberSource=()=>{const value=frame.getAttribute('src')||'';if(value.startsWith('blob:')&&!immutableSource)immutableSource=value};
    const sourceObserver=new MutationObserver(rememberSource);sourceObserver.observe(frame,{attributes:true,attributeFilter:['src']});
    ...
    const onFrameLoad=()=>{
      rememberSource();
      trustedLoad=false;quarantine(true);
