...
> **Trigger:** Invoked by `/openplanr-pipeline:po-phase` if `DatabaseType` is configured and `output/db/schema.json` is missing or stale; can also be invoked manually.
