...
description: Use this agent when decomposing a functional spec into User Stories + tasks for feat-{name}. Reads spec/stack/schema/design and writes the full feature arborescence under output/feats/. No code generation.
...
> **Trigger:** Invoked by `/openplanr-pipeline:po-phase` after upstream agents complete
> **Chained after:** db-agent (if DatabaseType configured) → designer-agent (if PNGs present)
> **Input feature name:** Passed by `/po-phase` as `$ARGUMENTS` (e.g. `auth` → operates on `feat-auth`)
...
3. Check if output/feats/feat-$ARGUMENTS/design-spec.md exists → set has_design = true/false
   (Designer Agent should have run first via /po-phase if PNGs were present)
4. Check if output/db/schema.json exists → load if relevant
   (DB Agent should have run first via /po-phase if DatabaseType is configured)
...
8. STOP. Do not proceed to DEV phase. The /po-phase orchestrator stops here for human review.
```
