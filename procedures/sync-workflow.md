# Procedure: Sync workflow — spec ↔ quick-task ↔ tracker reconciliation

> Read by `commands/sync.md` (thin router) and mirrored by the portable `planr-sync`
> skill. Both surfaces reach this one procedure — the sync workflow is implemented once,
> here. Read-only with respect to `.planr/` unless the operator passes `--apply` / `--push`.
