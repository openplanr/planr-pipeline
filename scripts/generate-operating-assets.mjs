...
const ROLE_INSTRUCTION_TEMPLATE = 'templates/runtime/operating-role-instruction.md.tpl';
...
function renderOperatingRoleDocs(registry) {
  const rows = registry.roles.map((role) => {
    ...
    return `| ${role.order} | ${role.id} | ${role.displayLabel} | ${markdownCell(role.mandate)} | ${role.capabilityTier} | ${role.allowedRouteKinds.join(', ')} | ${role.budgets.maxActions} | ${markdownCell(focus)} | ${markdownCell(role.investigationMandate.sufficientGrounding)} | ${markdownCell(role.forbiddenRecommendationCategories.join(', '))} |`;
  });
 ...
 */
function assertRoleRegistryV14(roles) {
  assertProtocolArtifact('operating-role-registry', roles, { protocolVersion: '1.4.0' });
}
...
export function renderPortableOperatingRoleAssets(roles, { projectRoot = root } = {}) {
  assertRoleRegistryV14(roles);
  const template = readCanonicalText(projectRoot, ROLE_INSTRUCTION_TEMPLATE);
  const assets = {};
  for (const role of lensRolesInOrder(roles)) {
    const rendered = template
      .replaceAll('{{ROLE_ID}}', role.id)
      .replaceAll('{{DISPLAY_LABEL}}', role.displayLabel)
      .replaceAll('{{MANDATE}}', role.mandate);
    assets[`adapters/codex/skills/planr-operate/references/roles/${role.id}.md`] = rendered;
    assets[`adapters/cursor/rules/openplanr-operate-roles/${role.id}.md`] = rendered;
  }
  for (const procedure of ['bootstrap', 'advisor', 'chair', 'review']) {
    const bytes = readCanonicalText(projectRoot, `procedures/operate/${procedure}.md`);
    assets[`adapters/codex/skills/planr-operate/references/${procedure}.md`] = bytes;
    assets[`adapters/cursor/rules/openplanr-operate-procedures/${procedure}.md`] = bytes;
  }
  return assets;
}
...
export function renderOperatingAssets({ projectRoot = root } = {}) {
  ...
  Object.assign(assets, renderPortableOperatingRoleAssets(roles, { projectRoot }));
