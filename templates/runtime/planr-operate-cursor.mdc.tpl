...
`native-isolated` advisor retains empty-tool isolation. Return exactly the
compact object described by `rolePack.roleBrief.output.jsonSchema`; do not add
canonical result metadata. Submit that `operating-advisor-response@1.2.0`
object through the CLI, finalize independent results, and prepare the Chair
separately. Never improvise or role-play beyond the supplied evidence.
