# OpenPlanr Protocol — Runtime Adapters (v1.0.0)

> How each AI coding agent runtime implements the protocol. Three adapters today; the protocol is open for additional runtime integrations.

## Adapter contract

A runtime adapter implements the protocol if and only if:

1. It exposes PLAN and SHIP as user-invokable commands.
2. PLAN and SHIP execute the orchestration described in `commands.md`.
3. PLAN does NOT auto-chain to SHIP (rule R1).
4. The 8 named roles in `agent-roles.md` are dispatched as separate execution contexts (subagents, personas, or equivalent).
5. The artifacts produced — SPEC, US, Task, design-spec, qa-report, error-report, `.pipeline-shipped` — match `spec-artifacts.md` byte-for-byte (modulo timestamp).
6. The `.pipeline-shipped` marker `runtime` field correctly identifies the runtime.

The conformance test fixture (`openplanr-pipeline/conformance/`) verifies items 5 and 6 mechanically. Items 1-4 require human inspection of the runtime's behaviour against the fixture.

## Claude Code adapter (canonical)

**Repo:** `openplanr/openplanr-pipeline`
**Install:** `/plugin marketplace add openplanr/marketplace && /plugin install openplanr-pipeline@openplanr`
**Adapter version:** lockstep with protocol version

### Implementation

- **Slash commands:** `commands/plan.md` and `commands/ship.md` registered via Claude Code plugin manifest
- **Subagents:** `agents/{role}.md` files with YAML frontmatter declaring `name`, `description`, `tools`, `model`. Tool restrictions enforced at the manifest layer — agent literally cannot invoke disallowed tools.
- **Hooks:** `hooks/hooks.json` registers a Stop hook for the `.snapshot-pending` reminder.
- **Templates:** `templates/{spec,spec-driven,stack,error-report,CLAUDE.md}.tpl` referenced via `${CLAUDE_PLUGIN_ROOT}` from commands and agents.

### Strengths

- Full manifest-enforced tool restrictions (R8, R9 enforced at the runtime layer, not just prompt)
- Stop hook provides safety net for snapshot reminder
- First-class slash command surface; no naming collision with built-ins (after v0.5.0 cleanup)

### Caveats

- Some users may not have Claude Code; this adapter doesn't help them.

## Cursor adapter

**Generated by:** `planr rules generate --target cursor --scope pipeline`
**Lives at:** `.cursor/rules/openplanr-pipeline.mdc` + `.cursor/rules/openplanr-pipeline-{plan,ship}.mdc` + `.cursor/rules/agents/*.md`
**Minimum runtime version:** Cursor 1.x with Composer subagent dispatch

### Implementation

- **Master rule:** `openplanr-pipeline.mdc` with `globs: [".planr/specs/**", "input/specs/**", "output/feats/**"]` for context-aware auto-attach.
- **Phase rules:** `openplanr-pipeline-plan.mdc` and `openplanr-pipeline-ship.mdc` activate when the user types matching keywords ("plan {feature}" / "ship {feature}").
- **Agent body files:** `.cursor/rules/agents/{role}.md` — system prompts vendored from `openplanr-pipeline/agents/` with frontmatter stripped. Cursor's Composer reads these for subagent dispatch.

### Strengths

- Native Cursor 1.x subagent dispatch (verified empirically)
- Auto-attach via `globs` — pipeline rules surface only when relevant files are open
- Same `.planr/specs/` artifacts as Claude Code; cross-runtime spec portability

### Caveats

- **Tool restrictions are advisory (prompt-level only).** Cursor's permission model is repo-level, not per-persona. The conformance test catches violations via post-ship git-diff on Preserve paths.
- **No Stop hook equivalent.** Snapshot reminder uses `.cursor/.snapshot-pending` sentinel surfaced on next session start.
- **Subagent dispatch fragility.** This is an emergent Cursor capability, not a documented primitive. Pinned to Cursor 1.x; behaviour may shift.
- `${CLAUDE_PLUGIN_ROOT}` references in upstream agent bodies are documented but not resolved — Cursor reads `.cursor/rules/agents/*.md` and `.cursor/rules/templates/*` directly.

## Codex adapter

**Generated by:** `planr rules generate --target codex --scope pipeline`
**Lives at:** `AGENTS.md` (project root) with a `## OpenPlanr Pipeline Orchestration` section appended
**Minimum runtime version:** Codex 2.0+

### Implementation

- **Single file:** Codex's canonical entry point is `AGENTS.md`. The pipeline section is concatenated onto the agile-mode body (when `--scope all`) or stands alone (when `--scope pipeline`).
- **Personas:** the 8 named roles are modelled as personas the model adopts during specific tasks. Codex doesn't have separate subagent processes — persona shifts happen in-session.
- **Mode detection:** identical algorithm to other adapters (read `.planr/config.json`, check `idPrefix.spec`).

### Strengths

- Zero install — `AGENTS.md` is Codex's native entry point
- Same artifact contract as other adapters
- Coexists with planr CLI's agile-mode AGENTS.md content (concatenation, not overwrite, when `--scope all`)

### Caveats

- **No subagent isolation.** Persona role-shift is sequential within one model context. Cross-task contamination possible if the model doesn't reset between dispatches.
- **Tool restrictions are advisory (prompt-level only).** Same as Cursor — conformance test catches via Preserve check.
- **No Stop hook equivalent.** Snapshot reminder is a soft prompt-level reminder.
- **Codex 2.0 persona quality unmeasured against the conformance fixture.** Documented as a v2 polish item; v1 ships with this acknowledged gap.

## Compatibility quick-reference

| Capability | Claude Code | Cursor | Codex |
|---|---|---|---|
| `PLAN` / `SHIP` orchestration | ✅ slash command | ✅ rule auto-attach | ✅ persona-triggered |
| Subagents (8 roles) | ✅ manifest-declared | ⚠️ Composer dispatch | ⚠️ persona role-shift |
| Manifest tool restrictions | ✅ enforced | ❌ advisory only | ❌ advisory only |
| Spec-driven mode | ✅ | ✅ | ✅ |
| Auto-scaffold spec shell | ✅ | ✅ | ✅ |
| Self-heal stack.md | ✅ | ✅ | ✅ |
| 3-iteration correction loop | ✅ | ✅ (prompt-driven) | ✅ (prompt-driven) |
| `.pipeline-shipped` marker | ✅ | ✅ | ✅ |
| CLAUDE.md snapshot | ✅ via Stop hook | ⚠️ no Stop hook | ⚠️ no Stop hook |
| R1 PLAN/SHIP separation | ✅ two commands | ✅ two rules | ✅ two keywords |
| Conformance test passes | ✅ | ✅ (with caveats) | ✅ (with caveats) |

Full matrix with caveats: `../compatibility-matrix.md`.

## Adding a new runtime adapter

To add a fourth runtime (e.g., Aider, Cline, Continue):

1. Implement the contract above (PLAN, SHIP, role dispatch, R1).
2. Generate adapter config files via a new generator class in `OpenPlanr/src/generators/{name}-generator.ts`.
3. Add a new `TargetCLI` value to `OpenPlanr/src/models/types.ts`.
4. Vendor the agent body files (or reference the openplanr-pipeline plugin cache).
5. Run the conformance test fixture against the new adapter.
6. Submit a PR to `openplanr-pipeline` with the adapter spec and matrix entry.

The protocol is open. New runtimes are welcome.

---

*OpenPlanr Protocol v1.0.0 — runtime adapter specs.*
