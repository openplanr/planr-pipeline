...
Run procedure: `${CLAUDE_PLUGIN_ROOT}/commands/procedures/mode-detection.md`. After it executes, `MODE` and (for spec-driven mode) `SPEC_DIR` are bound. The procedure also handles the self-heal-on-missing-`stack.md` pathway and the path-resolution table.

### 1b — Validate required inputs

The procedure file at `${CLAUDE_PLUGIN_ROOT}/commands/procedures/mode-detection.md` (section **Required inputs (per command) → /plan**) covers the required-inputs validation for both modes. After it returns:

- If MODE is `spec-driven` AND `<SPEC_DIR>/SPEC-NNN-${SLUG}.md` is missing, do NOT abort — run `${CLAUDE_PLUGIN_ROOT}/commands/procedures/auto-scaffold-spec.md` (the pipeline plugin is self-sufficient and does not require the planr CLI; the procedure decides whether to continue to Step 2 or abort gracefully based on whether `BRIEF` was substantive).
...
- [ ] If a stash from `STAGE_DESIGN_ASSETS` was created, `${CLAUDE_PLUGIN_ROOT}/commands/procedures/restore-design-assets.md` has run and the stash dir has been deleted
